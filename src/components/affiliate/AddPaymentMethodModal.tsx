...
const countries: Country[] = [
    { value: 'India', label: 'India', code_type: 'ifsc', code_label: 'IFSC Code', code_placeholder: 'SBIN0001234' },
    { value: 'United States', label: 'United States', code_type: 'routing', code_label: 'Routing Number', code_placeholder: '123456789' },
    { value: 'Canada', label: 'Canada', code_type: 'routing', code_label: 'Routing Number', code_placeholder: '123456789' },
    { value: 'United Kingdom', label: 'United Kingdom', code_type: 'iban', code_label: 'IBAN', code_placeholder: 'GB82WEST12345698765432' },
    { value: 'Germany', label: 'Germany', code_type: 'iban', code_label: 'IBAN', code_placeholder: 'DE89370400440532013000' },
    { value: 'France', label: 'France', code_type: 'iban', code_label: 'IBAN', code_placeholder: 'FR1420041010050500013M02606' },
    { value: 'International', label: 'International (Others)', code_type: 'swift', code_label: 'SWIFT Code', code_placeholder: 'DEUTDEFF' },
];

const AddPaymentMethodModal: React.FC<AddPaymentMethodModalProps> = ({ isOpen, onClose, onSuccess, editMode = false, methodId, initialData }) => {
        ...
        if (isOpen && editMode && initialData) {
            if (initialData.payment_type === 'paypal') {
                ...
                setFormData((prev) => ({
                    ...
                    account_number: initialData.account_number && !String(initialData.account_number).includes('*') ? initialData.account_number : '',
    ...
    const validateAccountHolderName = (name: string): string | null => {
        ...
        if (!/^[a-zA-Z\s.'-]+$/.test(name)) return 'Only letters, spaces, dots, hyphens allowed';
        return null;
    };
    ...
    const validateIFSC = (code: string): string | null => {
        ...
        if (!/^[A-Z]{4}0[A-Z0-9]{6}$/i.test(code)) return 'Invalid IFSC (e.g., SBIN0001234)';
        return null;
    };
    ...
    const validateIBAN = (iban: string): string | null => {
        ...
        if (!/^[A-Z]{2}[0-9]{2}[A-Z0-9]{11,30}$/.test(clean)) return 'Invalid IBAN format';
        return null;
    };

    const validateSWIFT = (code: string): string | null => {
        if (!code.trim()) return 'SWIFT code is required';
        if (!/^[A-Z]{4}[A-Z]{2}[A-Z0-9]{2}([A-Z0-9]{3})?$/i.test(code)) return 'Invalid SWIFT (8 or 11 chars)';
        return null;
    };
    ...
    const validateForm = (): boolean => {
        ...
        if (activeTab === 'bank_transfer') {
            ...
            if (selectedCountry) {
                ...
                const codeValue = formData[codeType === 'ifsc' ? 'ifsc_code' :
                    codeType === 'routing' ? 'routing_number' :
                        codeType === 'iban' ? 'iban' : 'swift_code'];
                ...
                if (codeErr) {
                    newErrors[codeType === 'ifsc' ? 'ifsc_code' :
                        codeType === 'routing' ? 'routing_number' :
                            codeType === 'iban' ? 'iban' : 'swift_code'] = codeErr;
                }
            }
        }
        ...
        try {
            ...
            if (activeTab === 'bank_transfer') {
                ...
                if (selectedCountry?.code_type === 'ifsc') {
                    ...
                    payload.routing_number = formData.routing_number.replace(/[\s-]/g, '');
            ...
            const response = editMode && methodId
                ? await updatePaymentMethod(String(methodId), payload)
                : await addPaymentMethod(payload);

            if (response.success) {
                showToast({
                    message: editMode ? 'Payment method updated successfully!' : 'Payment method added successfully!',
                    type: "success",
                    duration: 3000,
                });
                onSuccess();
                ...
                showToast({
                    message: response.error?.message || (editMode ? 'Failed to update payment method' : 'Failed to add payment method'),
                    type: "error",
                    duration: 3000,
                });
            }
                        ...
                        <div className="w-full lg:w-[65%] h-full pb-10 lg:pb-0 flex flex-col gap-[18px]">
                            <div className="flex flex-col gap-2">
                                <h2 className="text-2xl font-semibold font-['Poppins',Helvetica] bg-[linear-gradient(90deg,#6340FF_0%,#D748EA_100%)] bg-clip-text text-transparent">
                                    {editMode ? 'Update' : 'Add'} {paymentTabs.find((t) => t.id === activeTab)?.label}
                                </h2>
                            ...
                            <form onSubmit={handleSubmit} className="flex flex-col gap-4 h-full">
                                <div className="space-y-4 flex-1 overflow-y-auto ps-1 pr-2">
                                        ...
                                        <>
                                            <div>
                                                ...
                                                <input
                                                    ...
                                                    className={`w-full border ${errors.account_holder_name ? 'border-red-500' : 'border-[#ECEEF2]'} rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-[#7077FE] focus:border-transparent outline-none font-['Open_Sans',Helvetica]`}
                                                    placeholder="Enter account holder name"
                                                />
                                                {errors.account_holder_name && (
                                                    <p className="text-red-500 text-xs mt-1 font-['Open_Sans',Helvetica]">{errors.account_holder_name}</p>
                                                )}
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <Select
                                                    ...
                                                    styles={{
                                                        control: (base) => ({
                                                            ...base,
                                                            borderColor: errors.country ? '#ef4444' : '#ECEEF2',
                                                            '&:hover': {
                                                                borderColor: '#7077FE',
                                                            },
                                                        }),
                                                ...
                                                />
                                                {errors.country && (
                                                    <p className="text-red-500 text-xs mt-1 font-['Open_Sans',Helvetica]">{errors.country}</p>
                                                )}
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <input
                                                    ...
                                                    className={`w-full border ${errors.account_number ? 'border-red-500' : 'border-[#ECEEF2]'} rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-[#7077FE] focus:border-transparent outline-none font-['Open_Sans',Helvetica]`}
                                                    placeholder="Enter account number"
                                                />
                                                {errors.account_number && (
                                                    <p className="text-red-500 text-xs mt-1 font-['Open_Sans',Helvetica]">{errors.account_number}</p>
                                                )}
                                            </div>
                                            ...
                                            {selectedCountry && (
                                                <div>
                                                    <label className="block mb-2 font-medium text-sm text-[#222224] font-['Poppins',Helvetica]">
                                                        {selectedCountry.code_label} <span className='text-red-600'>*</span>
                                                    </label>
                                                    <input
                                                        type="text"
                                                        value={
                                                            selectedCountry.code_type === 'ifsc' ? formData.ifsc_code
                                                                : selectedCountry.code_type === 'routing' ? formData.routing_number
                                                                    : selectedCountry.code_type === 'iban' ? formData.iban
                                                                        : formData.swift_code
                                                        }
                                                        onChange={(e) => {
                                                            const field = selectedCountry.code_type === 'ifsc' ? 'ifsc_code'
                                                                : selectedCountry.code_type === 'routing' ? 'routing_number'
                                                                    : selectedCountry.code_type === 'iban' ? 'iban'
                                                                        : 'swift_code';
                                                            handleInputChange(field, e.target.value);
                                                        }}
                                                        className={`w-full border ${errors[selectedCountry.code_type === 'ifsc' ? 'ifsc_code' : selectedCountry.code_type === 'routing' ? 'routing_number' : selectedCountry.code_type === 'iban' ? 'iban' : 'swift_code'] ? 'border-red-500' : 'border-[#ECEEF2]'} rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-[#7077FE] focus:border-transparent outline-none font-['Open_Sans',Helvetica]`}
                                                        placeholder={selectedCountry.code_placeholder}
                                                    />
                                                    {errors[selectedCountry.code_type === 'ifsc' ? 'ifsc_code' : selectedCountry.code_type === 'routing' ? 'routing_number' : selectedCountry.code_type === 'iban' ? 'iban' : 'swift_code'] && (
                                    ...
                                    {activeTab === 'paypal' && (
                                        <div>
                                            ...
                                            <input
                                                ...
                                                className={`w-full border ${errors.paypal_email ? 'border-red-500' : 'border-[#ECEEF2]'} rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-[#7077FE] focus:border-transparent outline-none font-['Open_Sans',Helvetica]`}
                                                placeholder="your@email.com"
                                            />
                                            {errors.paypal_email && (
                                                <p className="text-red-500 text-xs mt-1 font-['Open_Sans',Helvetica]">{errors.paypal_email}</p>
                                            )}
                                        </div>
                                    )}
                                    ...
                                    <div className="flex items-center gap-2 pt-2">
                                        ...
                                        />
                                        <label htmlFor="is_default" className="text-sm text-[#222224] font-['Open_Sans',Helvetica]">
