  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-6" style={{ height: "57px" }}>
        ...
        {label && (
          <span className="text-xs px-2 py-1 bg-[#9747FF1A] text-[#9747FF] rounded-full">
            {label}
          </span>
        )}
        <button
      ...
      <div className="flex flex-col items-center text-center px-6">
        ...
        {/* Edit Button */}
        <button className="mt-3 px-4 py-2 bg-[#9747FF] text-white text-sm rounded-full hover:bg-[#8635e8] transition">
          Edit Profile
        </button>
        ...
        {icons.length > 0 && (
          <div className="flex gap-3 mt-4">
            {icons.map((icon, index) => (
              <img key={index} src={icon} alt={`icon-${index}`} className="w-5 h-5" />
            ))}
          </div>
        )}
      </div>
      ...
      <div className="flex items-center px-6 gap-4">
        ...
        <div className="flex flex-wrap gap-3 text-sm text-teal-400">
          {hashtags.map((tag) => (
            <span key={tag} className="cursor-pointer hover:underline">
              #{tag}
            </span>
          ))}
        </div>
          ...
          >
            ...
            {activeTab !== tab && (
              <span
                aria-hidden
                className="absolute top-0 left-0 right-0 bottom-0 rounded-t-[12px]
                  bg-gradient-to-b from-[#FFFFFF] via-[#F5F2FF] to-[rgba(151,71,255,0.14)] z-0 opacity-0 group-hover:opacity-100 transition-opacity"
              />
            )}
            ...
            {activeTab !== tab && (
              <span className="absolute bottom-0 left-0 right-0 h-[2px] bg-[#9747FF] rounded-full opacity-0 group-hover:opacity-100 transition-opacity" />
            )}
          </button>
        ))}
      </div>
