...
import {
  ...
  X,
 Bell, MoreHorizontal,
   Plus, ArrowLeft, ArrowRight, UserPlus, ArrowUpRight
} from "lucide-react";
...
/* ---------- Theme ---------- */
const GRADIENT =
  "bg-[linear-gradient(90deg,#7077FE_0%,#F07EFF_100%)]";
...
export function TrueProfileCard({
  ...
  return (
    <Card className="p-4 md:p-5"> {/* removed fixed height */}
      {/* Header */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="inline-flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-full">
            <img src={profileicon} alt="Profile Icon" className="h-5 w-5 sm:h-6 sm:w-6" />
          </span>
...
export function CertificationCard({
  ...
  const dotCls = (on: boolean) =>
    `h-1.5 w-1.5 rounded-full ${on ? "bg-[#7E5FFF]" : "bg-[#D8D6FF]"} transition-colors`;

return (
    <Card className="rounded-[12px] border border-[#E5E7EB] px-4 md:px-[18px] py-5 space-y-3"> {/* no fixed height */}
      {/* Header */}
        ...
        >
          ...
          <div
            className={`absolute inset-0 transition-opacity duration-500 ${
              slide === 0 ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
            }`}
          >
            ...
            <div className="mt-2 grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4 px-1 sm:px-2">
              ...
              >
                <img src="https://cdn.cness.io/aspiring.webp" alt="Aspired" className="h-[34px] w-[34px] sm:h-[39px] sm:w-[39px]" />
              ...
              <div
                className={`w-full h-[120px] sm:h-[150px] rounded-[18px] p-[2px] ${
                  activeLevel === "Inspired" ? "bg-gradient-to-r from-[#7077FE] to-[#F07EFF]" : "bg-[#E5E7EB]"
                }`}
              >
                <div className="w-full h-full rounded-[16px] bg-white flex flex-col items-center justify-center gap-[10px] sm:gap-[12px] px-4 py-4">
                  <img src="https://cdn.cness.io/inspired.webp" alt="Inspired" className="h-[34px] w-[34px] sm:h-[39px] sm:w-[39px]" />
                  <span className="font-poppins font-semibold text-[15px] sm:text-[18px] leading-[100%] text-[#0F1728]">
                    Inspired
                  </span>
                </div>
              ...
              >
                <img src="https://cdn.cness.io/leader.webp" alt="Leader" className="h-[34px] w-[34px] sm:h-[39px] sm:w-[39px]" />
                <span className="font-poppins font-semibold text-[15px] sm:text-[18px] leading-[100%] text-[#0F1728]">
                  Leader
                </span>
              </div>
          ...
          <div
  className={`absolute inset-0 transition-opacity duration-500 ${
    slide === 1 ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
  }`}
>
       ...
       className="mt-4 sm:mt-5 inline-flex w-full sm:w-auto min-w-[140px] h-10 sm:h-[40px] items-center justify-center gap-[7px] rounded-full bg-[#7077FE] px-5 sm:px-6 font-opensans text-[13px] sm:text-[14px] leading-[100%] text-white shadow hover:bg-[#5A61E8] transition"
        ...
        <div className="pt-3 flex justify-center">
          <div className="flex gap-1.5">
            <button aria-label="Slide 1" className={dotCls(slide === 0)} onClick={() => setSlide(0)} />
            <button aria-label="Slide 2" className={dotCls(slide === 1)} onClick={() => setSlide(1)} />
          </div>
        </div>
      </div>
    </Card>
          ...
          {items.map((bp) => (
            ...
            >
              ...
              <div className="h-[135px] rounded-[8px] overflow-hidden">
                <img src={bp.image} alt={bp.title} className="h-full w-full object-cover" />
              </div>
...
export function SocialStackCard({
  ...
  suggested: { id: string | number; name: string; handle: string; avatar: string }[];
  requested: { id: string | number; name: string; handle: string; avatar: string }[];
  onConnect?: (f: { id: string | number; name: string; handle: string; avatar: string }) => void;
}) {
...
function MarqueeColumn({
  ...
  return (
       <div aria-hidden className={`absolute inset-y-0 ${sideClass} w-12 overflow-hidden pointer-events-none hidden sm:block`}>

          <div className={`flex flex-col gap-2 ${reverse ? "marquee-ping-reverse" : "marquee-ping"}`}>

        {list.map((src, i) => (
                    <img key={`${src}-${i}`} src={src} className="h-10 w-10 rounded-lg object-cover border border-white/60 shadow" />
...
function AdventureSlides({
  ...
  const ReactionBubble = ({
      ...
      src?: string;
      className?: string;
    }) => (src ? <img src={src} className={`absolute h-8 w-8 object-contain drop-shadow-md ${className}`} /> : null);
    ...
    <div className="row-start-1 relative z-10 place-self-center w-full max-w-[620px]">
      ...
      <div className="mt-2 rounded-2xl border border-[#EEF0F5] bg-white p-5 shadow-sm">
        ...
        <div className="flex items-start justify-between gap-6">
          <div>
            <div className="font-poppins font-semibold text-[20px] leading-[32.3px] tracking-[0.15px] text-center text-[#0F1728]">Account Reached</div>
            <div className="mt-2 text-[18px] font-semibold text-[#F07EFF]">12,890</div>
          </div>
  ...
  <div className="flex flex-col gap-1 justify-end">
    {Array.from({ length: 4 }).map((_, i) => (
      <span key={i} className="h-2 w-2 rounded-full bg-[#F07EFF]" />
        ...
        <div className="flex items-start justify-between gap-6">
          <div>
            <div className="font-poppins font-semibold text-[20px] leading-[32.3px] tracking-[0.15px] text-center text-[#0F1728]">Followers</div>
            <div className="mt-2 text-[18px] font-semibold text-[#8B7CFF]">+256</div>
          </div>
...
const NotificationsCard = () => (
    <div className="row-start-1 relative z-10 place-self-center w-full max-w-[620px]">
      ...
      <div className="flex items-center gap-3">
        ...
        <h3 className="font-poppins font-semibold text-[20px] leading-[32.3px] tracking-[0.15px] text-center text-[#0F1728]">Notification</h3>
      </div>

      {/* rows */}
      <div className="mt-2 space-y-3">
        {/* Row 1 — request with CTA */}
        <div className="flex items-center justify-between rounded-2xl border border-[#EEF0F5] bg-white p-3 shadow-sm">
          <div className="flex items-center gap-3 min-w-0">
            <img src="https://i.pravatar.cc/56?img=64" className="h-11 w-11 rounded-full object-cover" />
            <div className="min-w-0">
              <div className="truncate text-[14px] font-semibold text-[#0F1728]">Gia Monroe</div>
              <div className="truncate text-[14px] text-[#98A2B3]">@gia.monroe_</div>
            </div>
          </div>
          <button className="h-9 rounded-full bg-white px-4 text-[12px] font-semibold text-[#1F2A37] border border-[#E6EAF0] shadow-sm">
            Accept Request
          </button>
        </div>
        ...
        <div className="flex items-center justify-between rounded-2xl border border-[#EEF0F5] bg-white p-3 shadow-sm">
          <div className="flex items-center gap-3 min-w-0">
            <img src="https://i.pravatar.cc/56?img=45" className="h-11 w-11 rounded-full object-cover" />
            <div className="min-w-0">
              <div className="truncate text-[14px] font-semibold text-[#1F2937]">
                Lara and sunil <span className="font-normal text-[#8F9AA6]">liked your act</span>
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between rounded-2xl border border-[#EEF0F5] bg-white p-3 shadow-sm">
          <div className="flex items-center gap-3 min-w-0">
            <img src="https://i.pravatar.cc/56?img=41" className="h-11 w-11 rounded-full object-cover" />
            <div className="min-w-0">
              <div className="truncate text-[14px] font-semibold text-[#1F2937]">
                Maya and Arjun{" "}
                <span className="font-normal text-[#8F9AA6]">appreciated your performance</span>
              </div>
            </div>
          </div>
          <div className="flex h-9 w-9 items-center justify-center rounded-full border border-[#EEF0F5] bg-white shadow-sm">
      ...
      {/* CTA */}
      <button className="mt-4 inline-flex h-10 w-full items-center justify-center rounded-full text-white text-[14px] font-semibold bg-[linear-gradient(90deg,#7077FE_0%,#9747FF_60%,#F07EFF_100%)] shadow">
        ...
        >
          ...
          {idx === 0 && (
            <>
              ...
              <ReactionBubble src={reactions?.topRight} className="right-3 top-3" />
              <ReactionBubble src={reactions?.bottomLeft} className="left-3 bottom-6" />
            </>
          )}
          ...
          {idx === 0 ? (
            <div className="relative z-10 h-full grid place-items-center text-center">
              <div>
                <h4 className="font-poppins font-semibold text-[20px] leading-[32.3px] tracking-[0.15px] text-center text-[#0F1728]">{s.title}</h4>
                <p className="mt-2 max-w-[32rem] mx-auto text-[14px] leading-[100%] text-[#667085]  font-[400] text-center font-['Open_Sans']">{s.text}</p>
                <div className="mt-4 flex items-center justify-center gap-3">
                  <PrimaryButton onClick={onPrimary}>{s.primaryLabel ?? "Start Posting"}</PrimaryButton>
                  <OutlinePill onClick={onSecondary}>{s.secondaryLabel ?? "View Feed"}</OutlinePill>
                </div>
              </div>
            </div>
          ) : idx === 1 ? (
  ...
  return (
    <Card className="p-4 md:p-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-[#EAF4FF]">
            <img src={socialicon} alt="Best Practices Icon" className="h-10 w-10" />
          </span>
          <span className="font-poppins font-medium text-[16px] leading-[100%] text-[#0F1728]">
            Social Profile</span>
        </div>
...
<button
  ...
  <div className="flex items-center gap-3">
    ...
    <div className="ml-auto shrink-0 flex items-center gap-4 text-[14px] mr-20">
      <span className="whitespace-nowrap">
        <span className="font-poppins font-medium text-[16px] leading-[100%] text-[#7077FE]">{resonating}</span>
        <span className="ml-1 font-['Plus_Jakarta_Sans'] font-normal text-[11px] leading-[100%] text-[#667085]">Resonating</span>
      </span>
      <span className="whitespace-nowrap">
        <span className="font-poppins font-medium text-[16px] leading-[100%] text-[#A855F7]">
          {Intl.NumberFormat().format(resonators)}
        </span>
        <span className="ml-1 font-['Plus_Jakarta_Sans'] font-normal text-[11px] leading-[100%] text-[#667085]">Resonators</span>
      </span>
    </div>
  </div>
  ...
  {/* Button */}
  <button className="mt-auto w-full flex items-center justify-center gap-[7.09px] 
...
>
  ...
  <div className="flex items-center gap-2">
          ...
          <span className="text-[16px] font-medium font-Poppins text-[#222224]">Friends</span>
        </div>
...
>
  ...
  <div className="space-y-3 flex-1">
    {list.slice(0, 5).map((f) => (
      <div key={f.id} className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          <img src={f.avatar} className="h-9 w-9 rounded-full object-cover" />
          <div className="min-w-0">
            <div className="truncate text-sm font-semibold text-[#0F1728]">
              {f.name}
            </div>
            <div className="truncate text-xs text-[#667085]">{f.handle}</div>
          </div>
        </div>
        <OutlinePill className="h-9 px-3" onClick={() => onConnect?.(f)}>
  ...
  <PrimaryButton className="w-full rounded-3xl">See More </PrimaryButton>
</div>
    </Card>
  );
}
  ...
  return (
    <Card className="p-4 md:p-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-[#FFF4E5]">
          <img src={directoryicon} alt="directory Icon" className="h-10 w-10" />
