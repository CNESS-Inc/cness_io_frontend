    ...
    <>
      <AnimatePresence>
        {!showSecond && (
          <motion.div
            ref={firstLoadRef}
            key="first-load"
            className="first-load flex flex-col items-center space-y-8 py-24 px-8 bg-[#F7F7F7]"
            style={{ position: 'relative', zIndex: 2 }}
            initial={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -40 }}
            transition={{ duration: 0.7, ease: 'easeInOut' }}
          >
            <div className="py-24 px-0 flex justify-between max-w-[800px] w-full mx-auto">
              <div>
      ...
      <AnimatePresence>
        {showSecond && (
          <motion.div
            key="second-load"
            className="second-load flex flex-col items-center space-y-8 py-24 px-8 bg-[#F7F7F7]"
            ...
            <div className="max-w-[1336px] mx-auto">
              <div className="flex flex-col items-center gap-8 md:gap-[52px] w-full">
                  ...
                  {cards.map((card, index) => (
                    <SwiperSlide key={index}>
                      <div className="relative">
                        <Card className="flex flex-col h-full w-full rounded-[12px] overflow-hidden">
                          {/* Card image as <img> instead of background */}
                          <div className="relative w-full rounded-t-[12px] overflow-hidden" style={{ height: "250px" }}>
                            <img
                              src={card.image}
                              alt={card.altText}
                          ...
                          {/* Flash effect only on bottom text/content area */}
                          <div className="card-flash-area relative">
                            {isFlashing && (
                              <span className="flash-blink-effect" />
                            )}
                            <CardContent className="flex flex-col w-full p-0">
                              <div className="flex flex-col gap-2 px-5 py-4 w-full rounded-b-[12px] bg-cover bg-center" style={{ backgroundImage: card.bg_image, marginTop: 0 }}>
                ...
                <div className="flex max-w-[750px] w-full mx-auto mt-6">
                  <p className="w-6/12 text-[#898989] text-[14px] font-regular">
                  ...
                  <div className="flex justify-end gap-6 w-6/12">
                    <Button
                      variant="outline"
                      className="bg-white h-[42px] border-[#2222241a] px-4 sm:px-6 py-1 rounded-[100px] text-[#222224] font-medium text-[14px] w-full sm:w-auto"
                    >
                      Start Selling
                    </Button>
                    <Button className="rounded-[100px] h-[42px] py-1 px-8 self-stretch text-[14px] bg-gradient-to-r from-[#7077FE] to-[#9747FF] text-white">
                      Browse Marketplace
                    </Button>
                  </div>
                </div>
