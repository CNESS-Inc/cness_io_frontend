...
const EditMusicForm: React.FC = () => {
    ...
    useEffect(() => {
        const fetchProductData = async () => {
            ...
            try {
                const response = await GetPreviewProduct('music', productId);
                ...
                if (productData) {
                    setFormData({
                        ...
                        total_duration: productData.music_details?.total_duration || "",
                        language: productData.language || "",
                        theme: productData.music_details?.theme || "",
                        format: productData.music_details?.format || "",
                    });
    ...
    const toggleEditFile = (trackId: any, fileOrderNumber: number) => {
        setTracks((prev) =>
            prev.map((track) =>
                track.id === trackId
                    ? {
    ...
    const handleSubmit = async (isDraft: boolean = false) => {
        ...
        try {
            ...
            showToast({
                message: isDraft ? "Music product saved as draft successfully" : "Music product updated successfully",
                type: "success",
                duration: 3000,
            });
            ...
            showToast({
                message: error?.message || error?.response?.data?.error?.message || 'Failed to update music product',
                type: "error",
                duration: 3000,
            });
        } finally {
    ...
    return (
        <>
            ...
            <div className="max-w-9xl mx-auto px-2 py-1 space-y-10">
                <FormSection
                    title="Edit Music"
                    description="Update your tracks, set pricing, and publish them to the marketplace."
                >
                        ...
                        <div>
                            ...
                            <div className="space-y-3">
                                ...
                                <p className="text-sm text-gray-500">
                                    Add up to 3 key highlights about your music
                                </p>
                            </div>
