...
<img
         src={missionIcon}// Replace with actual path
          ...
          className="w-10 h-10"
        />      </div>
      <h1 className="text-xl font-bold text-gray-800">{currentSection.name}</h1>
    </div>
...
<div className="px-4 pt-4 pb-10 max-w-6xl mx-auto space-y-6">
      <div className="bg-white rounded-3xl shadow-base p-8 space-y-10">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {currentSection.checkboxes.map((option) => (
              <label key={option.id} className="flex items-start gap-3 text-gray-700">
                <input
                  type="checkbox"
        ...
        <div className="space-y-6">
          <h3 className="text-lg font-semibold text-gray-700 pb-2 border-b border-gray-200">Purpose Pause</h3>
            ...
            <div key={i}>
              ...
              <textarea
               placeholder="Enter Text here"
                ...
                onChange={(e) => handlePurposePauseChange(i, e.target.value)}
              />
            </div>
          ))}
        </div>
...
className="w-full h-16 px-2 py-3 cursor-pointer rounded-xl border border-gray-300 file:mr-4 file:rounded-full file:border-0 file:bg-[#7077FE] file:px-4 file:py-2 file:text-white file:font-medium file:cursor-pointer"        
    onChange={(e) => handleFileUpload(i, e.target.files?.[0] || null)}
          ...
          <div>
            <label className="block font-medium text-sm text-gray-700 mb-1">Link for Reference</label>
            <input
              type="url"
          ...
          <>
            <div className="mt-10 border-t pt-6">
              <p className="text-gray-600 text-sm mb-3">
                Please ensure all required fields are completed before submission. Once submitted, your assessment will be reviewed within 5–7 business days. You won’t be able to make changes after submission.
              </p>
              <div className="flex justify-center">
                <Button onClick={handleconfirm}>Submit For Assessment</Button>
              </div>
            </div>
          </>
        )}
