...
const EditArtsForm: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      ...
      try {
        const response = await GetPreviewProduct('art', productId);
        ...
        if (productData) {
          setFormData({
            ...
            theme: productData.arts_details?.theme || "",
            mediums: productData.arts_details?.mediums || "",
            modern_trends: productData.arts_details?.modern_trends || "",
          });
  ...
  const handleDeleteChapter = async (chapterId: any) => {
    if (chapterId) {
      ...
      try {
        await DeleteArtChapter(productId, chapterId);
  ...
  const handleSubmit = async (isDraft: boolean = false) => {
    ...
    try {
            ...
            chapter.chapter_files.map(async (chapterFile) => {
              if (chapterFile.file) {
                ...
                try {
                  ...
                  return {
                    url: uploadData?.chapter_file_url || uploadData?.document_url || uploadData?.image_url || "",
      ...
      await UpdateArtProduct(payload, productId);

      const status = isDraft ? 'draft' : 'published';
      await UpdateProductStatus({ status }, productId);

      showToast({
        message: isDraft ? "Art product saved as draft successfully" : "Art product updated successfully",
        type: "success",
        duration: 3000,
      });
      ...
      showToast({
        message: error?.message || error?.response?.data?.error?.message || 'Failed to update art product',
        type: "error",
        duration: 3000,
      });
    } finally {
  ...
  return (
    <>
      ...
      <div className="max-w-9xl mx-auto px-2 py-1 space-y-10">
        <FormSection
          title="Edit Arts"
          description="Update your digital art collections, pricing, and publish changes to the marketplace."
        >
