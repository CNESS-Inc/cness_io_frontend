...
const EbookPreview = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showToast({
        message: "Ebook product submitted successfully!",
        type: "success",
        duration: 3000,
      });
    ...
    <div className="min-h-screen flex flex-col items-left py-2 px-2">
      ...
      <div className="bg-white rounded-xl shadow-md w-full p-6 mb-8">
        ...
        <div className="flex gap-10 text-gray-700 mb-2 flex-wrap">
          {data.ebook_details?.pages && (
            <div>
              <div className="flex items-center gap-1 mb-1 font-semibold">
                <FileText className="w-4 h-4" style={{ color: "#8B5CF6" }} />
                <span>Pages</span>
              </div>
              <span className="text-sm">{data.ebook_details.pages}</span>
            </div>
          )}
          {data.language && (
            <div>
              <div className="flex items-center gap-1 mb-1 font-semibold">
                <MdLanguage style={{ color: "#8B5CF6" }} />
                <span>Language</span>
              </div>
              <span className="text-sm" style={{ fontFamily: "Poppins" }}>{data.language}</span>
            </div>
          )}
          {data.ebook_details?.format && (
            <div>
              <div className="flex items-center gap-1 mb-1 font-semibold">
                <Book className="w-4 h-4" style={{ color: "#8B5CF6" }} />
                <span>Format</span>
              </div>
              <span className="text-sm" style={{ fontFamily: "Poppins" }}>{data.ebook_details.format}</span>
            </div>
          )}
          {data.ebook_details?.theme && (
            <div>
              <div className="flex items-center gap-1 mb-1 font-semibold">
                <span>🎨</span>
                <span>Theme</span>
              </div>
              <span className="text-sm" style={{ fontFamily: "Poppins" }}>{data.ebook_details.theme}</span>
            </div>
          )}
        </div>
      </div>
      ...
      {data.content_items && data.content_items.length > 0 && (
        <div className="bg-white rounded-xl shadow-md w-full p-6 mb-8">
          <h3 className="font-semibold text-lg mb-4" style={{ fontFamily: "Poppins" }}>Chapter List</h3>
          <div className="space-y-3">
            {data.content_items.map((chapter: any) => (
              <div key={chapter.id} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex-1">
                      <h4 className="font-semibold text-[#242E3A]">{chapter.title}</h4>
                      {chapter.description && (
                  ...
                  {chapter.file_urls && chapter.file_urls.length > 0 && (
                    <div className="text-sm text-gray-500">
                      {chapter.file_urls.length} file{chapter.file_urls.length > 1 ? 's' : ''}
                    </div>
                  )}
                </div>
                ...
                {chapter.file_urls && chapter.file_urls.length > 0 && (
                  <div className="mt-3 pl-14 space-y-2">
                    {chapter.file_urls.map((file: any, fileIndex: number) => (
      ...
      <div className="bg-white rounded-xl shadow-md w-full p-4 flex items-center gap-4">
        ...
        <div>
          ...
          <div className="text-sm text-gray-500">{data.product_category?.name || "Publisher"}</div>
        </div>
