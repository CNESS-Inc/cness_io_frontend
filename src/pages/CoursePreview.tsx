...
const CoursePreview = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showToast({
        message: "Course product submitted successfully!",
        type: "success",
        duration: 3000,
      });
    ...
    <div className="min-h-screen flex flex-col items-left py-2 px-2">
        ...
        <div className="flex flex-col md:flex-row">
          ...
          <div className="relative p-2 w-full">
            <img
              ...
              onError={() => {
                return (
                  <div className="rounded-xl w-full h-54 bg-gradient-to-br from-[#4F46E5] to-[#06B6D4] flex items-center justify-center">
      ...
      <div className="bg-white rounded-xl shadow-md w-full p-6 mb-8">
        ...
        <div className="flex gap-10 text-gray-700 mb-2 flex-wrap">
          {data.course_details?.duration && (
            <div>
              <div className="flex items-center gap-1 mb-1 font-semibold">
                <BsClockHistory style={{ color: "#4F46E5" }} />
                <span>Duration</span>
              </div>
              <span className="text-sm">{data.course_details.duration}</span>
            </div>
          )}
          {data.language && (
            <div>
              <div className="flex items-center gap-1 mb-1 font-semibold">
                <MdLanguage style={{ color: "#4F46E5" }} />
                <span>Language</span>
              </div>
              <span className="text-sm" style={{ fontFamily: "Poppins" }}>{data.language}</span>
            </div>
          )}
          {data.course_details?.format && (
            <div>
              <div className="flex items-center gap-1 mb-1 font-semibold">
                <Video className="w-4 h-4" style={{ color: "#4F46E5" }} />
                <span>Format</span>
              </div>
              <span className="text-sm capitalize" style={{ fontFamily: "Poppins" }}>{data.course_details.format}</span>
            </div>
          )}
        </div>
      </div>
      ...
      {data.course_details?.storytelling && (
        <div className="bg-white rounded-xl shadow-md w-full p-6 mb-8">
      ...
      {data.content_items && data.content_items.length > 0 && (
        <div className="bg-white rounded-xl shadow-md w-full p-6 mb-8">
          <h3 className="font-semibold text-lg mb-4" style={{ fontFamily: "Poppins" }}>Course Content</h3>
          <div className="space-y-3">
            {data.content_items.map((lesson: any) => (
              <div key={lesson.id} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex-1">
                      <h4 className="font-semibold text-[#242E3A]">{lesson.title}</h4>
                      {lesson.description && (
                ...
                {lesson.file_urls && lesson.file_urls.length > 0 && (
                  <div className="mt-3 pl-14 space-y-2">
                    {lesson.file_urls.map((file: any, fileIndex: number) => (
      ...
      {data.course_details?.requirements && (
        <div className="bg-white rounded-xl shadow-md w-full p-6 mb-8">
      ...
      <div className="bg-white rounded-xl shadow-md w-full p-4 flex items-center gap-4">
        ...
        <div>
          ...
          <div className="text-sm text-gray-500">{data.product_category?.name || "Course Instructor"}</div>
        </div>
